...
<xsl:stylesheet version="2.0"
    ...
    <!-- include this to invoke custom templates for header and footer
    <xsl:include href="custom-banner.xsl"/>
    -->
    ...
    <xsl:template name="geonetwork_banner">
        <tr class="banner">
            <td class="banner">
                <img src="{/root/gui/url}/images/header-left.jpg" alt="World picture" align="top"/>
            </td>
            <td align="right" class="banner" colspan="2">
    ...
    <xsl:template name="banner">
        ...
        <table width="100%">
            ...
            <!-- title
            <xsl:call-template name="custom_banner"/>
			-->
            <xsl:call-template name="geonetwork_banner"/>

            <!-- buttons -->
            <tr class="banner">
                <td class="banner-menu banner-menu-links">
                    <a class="banner" href="{/root/gui/locService}/home">
                        <xsl:value-of select="/root/gui/strings/home"/>
                    </a>
                    |
                    <xsl:if test="$modal">
                        <xsl:if test="java:isAccessibleService('metadata.add.form')">
                            <a class="banner" href="javascript:void(0)" onclick="doBannerButton('{/root/gui/locService}/metadata.create.form','{/root/gui/strings/newMetadata}',{$modal}, 600);">
                                <xsl:value-of select="/root/gui/strings/newMetadata"/>
                            </a>
                            |
                        </xsl:if>
                    </xsl:if>
                    <xsl:if test="string(/root/gui/session/userId)!=''">
                        <xsl:choose>
                            <xsl:when test="/root/gui/reqService='admin'">
                                <font class="banner-active">
                                    <xsl:value-of select="/root/gui/strings/admin"/>
                                </font>
                            </xsl:when>
                            <xsl:otherwise>
                                <a class="banner" onclick="doAdminBannerButton('{/root/gui/locService}/admin','{/root/gui/strings/admin}','{$modal}',800, 500)" href="javascript:void(0);">
                                    <xsl:value-of select="/root/gui/strings/admin"/>
                                </a>
                            </xsl:otherwise>
                        </xsl:choose>
                    ...
                    <xsl:choose>
                        <xsl:when test="/root/gui/reqService='feedback'">
                            <font class="banner-active">
                                <xsl:value-of select="/root/gui/strings/contactUs"/>
                            </font>
                        </xsl:when>
                        <xsl:otherwise>
                            <a class="banner" onclick="doBannerButton('{/root/gui/locService}/feedback','{/root/gui/strings/contactUs}','{$modal}',600)" href="javascript:void(0);">
                                <xsl:value-of select="/root/gui/strings/contactUs"/>
                            </a>
                        </xsl:otherwise>
                    </xsl:choose>
                    ...
                    <xsl:choose>
                        <xsl:when test="/root/gui/reqService='links'">
                            <font class="banner-active">
                                <xsl:value-of select="/root/gui/strings/links"/>
                            </font>
                        </xsl:when>
                        <xsl:otherwise>
                            <a class="banner" onclick="doBannerButton('{/root/gui/locService}/links','{/root/gui/strings/links}','{$modal}',600)" href="javascript:void(0);">
                                <xsl:value-of select="/root/gui/strings/links"/>
                            </a>
                        </xsl:otherwise>
                    </xsl:choose>
                    ...
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <a class="banner" onclick="doBannerButton('{/root/gui/locService}/about','{/root/gui/strings/about}','{$modal}',800)" href="javascript:void(0);">
                                <xsl:value-of select="/root/gui/strings/about"/>
                            </a>
                        </xsl:otherwise>
                    </xsl:choose>
                    ...
                    <xsl:choose>
                        <xsl:when test="java:file-exists(concat(resolve-uri('../', replace(static-base-uri(), 'file:', '')), 'docs/', /root/gui/language, '/users/index.html'))">
                            <a class="banner" href="{/root/gui/url}/docs/{/root/gui/language}/users" target="_blank">
                                <xsl:value-of select="/root/gui/strings/help"/>
                            </a>
                        </xsl:when>
                        <xsl:otherwise>
                            <a class="banner" href="{/root/gui/url}/docs/eng/users" target="_blank">
                                <xsl:value-of select="/root/gui/strings/help"/>
                            </a>
                        </xsl:otherwise>
                    </xsl:choose>
                ...
                <td class="banner-menu banner-menu-lang">
                    <xsl:if test="count(/root/gui/config/languages/*) &gt; 1">
                        ...
                        <select class="banner-content content">
                            <xsl:attribute name="onchange">location.replace('../' + this.options[this.selectedIndex].value + '/<xsl:value-of select="$redirectTo"/>');
                            </xsl:attribute>
                ...
                <xsl:choose>
                    <xsl:when test="string(/root/gui/session/userId)!=''">
                        <td align="right" class="banner-login banner-menu-login">
                            <form name="logout" action="{/root/gui/url}/j_spring_security_logout" method="post">
                                ...
                                <button align="right" class="banner" onclick="doLogout()">
                    ...
                    <xsl:when test="string(/root/gui/reqService) = 'login.form'">
                        ...
                        <td align="right" class="banner-login banner-menu-login"></td>
                    </xsl:when>
                    <xsl:otherwise>
                        <td align="right" class="banner-login banner-menu-login">
                            <xsl:choose>
                                ...
                                <xsl:otherwise>
                                    ...
                                    <form name="login" method="post"
										action="{/root/gui/url}/j_spring_security_check?redirectUrl=/srv/{/root/gui/language}/{/root/gui/reqService}{$paramString}">
    ...
    <xsl:template mode="casParams" match="*">&amp;<xsl:value-of select="name(.)"/>
